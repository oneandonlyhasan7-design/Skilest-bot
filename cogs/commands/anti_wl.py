...
class Whitelist(commands.Cog):
    ...
    async def whitelist(self, ctx, member: discord.Member = None):
        ...
        is_owner = ctx.author.id == ctx.guild.owner_id or ctx.author.id == 1359085571250589839
        ...
        if not antinuke or not antinuke[0]:
            embed = discord.Embed(
                color=0x000000,
                description=(
                    f"**{ctx.guild.name} Security Settings <:shield:1465741375134765151>\n"
        ...
        if not member:
            ...
            embed.add_field(name="__**Usage**__", value=f"<a:nt_reddot:1371025246374592546><:red_dot:1222796144996777995> `{prefix}whitelist @user/id`\n<:red_dot:1222796144996777995> `{prefix}wl @user`")
            embed.set_thumbnail(url=ctx.bot.user.avatar.url)
            return await ctx.send(embed=embed)
        ...
        embed.set_footer(text=f"Developed by skilest devolopement™")
