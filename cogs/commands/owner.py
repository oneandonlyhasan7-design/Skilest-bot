    ...
    async def _ownerunban(self, ctx: Context, user_id: int, *, reason: str = "No reason provided"):
        ...
        if user:
            try:
                ...
                embed = discord.Embed(
                    title="Error!",
                    description=f"<:exclamation:1155866914120282162>  I do not have permission to unban **{user.name}** in this guild.",
                    color=0x000000
                )
                await ctx.reply(embed=embed, mention_author=False)
            except discord.HTTPException:
    ...
    async def badges(self, ctx, member: discord.Member = None):
        ...
        if has_badges:
            ...
            badge_mapping = {
              ...
              "partner": "<a:PartnerBadgeShining:1230031829923401761> Partnered Server Owner",
